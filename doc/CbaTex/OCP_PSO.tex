...
\begin{itemize}
			...
			\item Calcular fluxo de potência, tensão nas barras e as perda elétricas para todas as partículas da geração atual, utilizadas na construção de $\textbf{f}(x)$;
			\item Atualizar o valor inicial de pBest das partículas com o valor calculado no passo anterior;
			\item Atualizar o valor inicial de gBest com o menor resultado encontrado no passo anterior.
\end{itemize}
...
\ENSURE OCP é o último valor de gBest
...
\STATE[6] Se algum pBest for maior que $f(\textbf{x})$, atualizar;
\STATE[7] Encontrar partícula com menor pBest;
\STATE[8] Atualizar gBest com menor valor de pBest.
...
Durante o processo evolutivo da otimização, para um ganho de diversidade, foi elaborado um método de mutação das partículas. A uma taxa constante de probabilidade de 1\%, cada partícula pode sofrer uma alteração aleatória em seus valores. Isto ocorre para evitar que a resposta vá, prematuramente, à um ótimo local, o que prejudicaria muito o resultado final. Este resultado será composto de dois importantes critérios.

O primeiro destes critérios tem como foco a minimização de perdas elétricas do circuito. Esta é obtida através da soma da potência dissipada em todas as linhas do alimentador, conforme a equação \ref{eq_obj1-perdas}.
\begin{equation} \label{eq_obj1-perdas}
  PerdasGlobais = \sum^{numL}_{l=linhaInicial}perda_l
\end{equation}
Já a variação da tensão ao longo do circuito durante a alocação dos capacitores, pode ser mensurada com auxílio da Eq. \ref{eq_vdi}, denominada VDI (\textit{Voltage Deviation Index)}. Similar ao cálculo de ~\citeasnoun{abul2013optimal}. Estabilidade no nível de tensão, via otimização por PSO foi bastante explorada em ~\citeasnoun{kim2011optimal} e ~\citeasnoun{hernandez2006optimal}.
