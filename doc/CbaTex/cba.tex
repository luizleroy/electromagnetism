...
\begin{algorithm}[H]
	\begin{algorithmic}	
			...
			\State Realizar o cálculo do fluxo de potência de todos as soluções candidatas iniciais (as partículas), para obtenção da perda elétrica total no circuito, que corresponde ao resultado da função objetivo;
	...
	\While{(não atingir critério de parada)}
			\State Calcular o valor da constante de inércia definida em \ref{inert};
			\State Calcular os valores de velocidade para todas as soluções candidatas conforme \ref{vel};
			\State Calcular a nova posição da partícula para todas as novas soluções candidatas conforme \ref{pos};
			\State Realizar mutações, aleatórias, conforme um percentual pré-definido;
			\State Realizar o cálculo do fluxo de potência de todos as soluções candidatas para obter a perda elétrica;
		\For{(todas as partículas)}
			\If{(novo valor de perda for menor que o valor da iteração anterior)}
				\State Atualizar o valor de pbest;
			\EndIf
		\EndFor
		\State Encontrar o menor valor de pbest;
		\If{(menor pbest for menor que o valor da iteração anterior de gbest)}
			\State Atualizar o valor corrente de gbest com o menor valor de pbest;
		\Else
			\State Atualizar o valor corrente de gbest com o valor da iteração anterior;
		\EndIf
	\EndWhile
