...
Na literatura este tipo de problema é conhecido como
\emph{Optimal Capacitor Placement} - OCP.
\end{abstract}

\keywords{PSO, OCP, Otimização Clássica, inteligência de enxames, SEP.}
]
...
\begin{itemize}
 ...
 utiliza-se do cálculo do fluxo de carga da linha de distribuição e de um método
 de otimização (PSO) para a solução de um problema não linear. Para cada
 aplicação detalhes técnicos são requeridos para utlização do PSO. Muitas formas
...
O problema foi formulado buscando encontrar pontos ótimos de alocação de capacitores em redes de grande extensão.
 ...
 Nas próximas subseções, explica-se como é realizado na prática a solução para o OCP. Do ponto de vista cronológico
  é necessário calcular o fluxo de potência (Load Flow /CITCITCIT) e em seguida utilizar uma técnica de otimização para
   minimizar as perdas ao longo do circuito. É importante também, lembrar de restringir casos em que o desvio de tensão ocorre na rede.
...
A função objetivo a ser minimizada é simples. Basta somar as perdas ao longo do circuito, que serão minimizadas ao inserir
 ...
 respeitar exigências de normas do setor, conforme consta no PRODIST módulo Oito de ~\citeasnoun{de2010modulo}.

 Durante este trabalho os níveis de tensão foram seguidos de acordo com a faixa indicada no anexo I de ~\citeasnoun{de2010modulo}, 
 seguindo o critério mais próximo dos circuitos testados da seção sequente (Simulações e resultados).
...
Ela serve para controlar o uso de capacitores no controle da tensão ao longo da linha, 
evitando valores deteriorados de tensão conforme ~\citeasnoun{abul2014optimal}.
...
Abaixo explica-se a principal contribuição do procedimento contruído ao longo deste trabalho(a metodologia de otimização via PSO) e 
em seguida uma breve introdução sobre o principal método de obtenção do fluxo de carga.

\subsection{Metodologia de minimização por ,,,Particle Swarm Optimization,,, }
O tamanho do espaço de busca para o problema dos capacitores pode atingir dimensões enormes.
 É possível modelar o problema de forma que exista uma relação entre um valor n de barras candidatas 
 com uma quantidade q de capacitores disponíveis. Isto leva um conjunto de testes inviáveis para cálculos forma bruta
...
,,,Cálculo de fluxo por varredura diereta / reversar sweep /CITCITCIT,,,

\section{Algoritmo PSO}
Bla3\ldots

\section{Simulações e resultados}
Um,, comparar PSO com outras --->otimizações<--- de mercado
