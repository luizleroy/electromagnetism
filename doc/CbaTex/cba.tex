...
Através de uma técnica de otimização robusta, monoobjetivo, denominada PSO (Particle Swarm Optimization) apresenta-se um método para alocação bancos de capacitores.
...
\keywords{\textcolor[rgb]{1.00,0.00,0.00}{Sistema de Distribuição}, OCP, Redução de Perdas, Perfil de tensão, PSO, Inteligência de Enxames, Computação Evolucionaria.}
...
Fornecendo como benefício a redução de perdas ativas no sistema.
...
Na próxima seção, que descreve o método de cálculo para o fluxo elétrico de potência e o processo de otimização, explica-se como é realizado na prática a solução para o OCP. Em seguida, temos os resultados deste trabalho, bem como os requisitos computacionais. No final do artigo existe uma seção para as conclusões e outra para os agradecimentos às entidades que fomentaram a pesquisa.
...
Então, subtrai-se a queda de tensão em cada barra até chegar às extremidades.
...
Com auxílio de um método de cálculo de fluxo de carga, e a otimização por enxames de partículas (PSO), é possível contruir o procedimento de alocação de dispositivos capacitivos na linha de distribuição (OCP) conforme consta no algoritmo abaixo:
...
\begin{algorithm}[H]
	\begin{algorithmic}	
			...
			\State Atualizar o valor inicial de pbest das partícuals com o valor calculado no passo imediatamente anterior;
...
circuito durante a alocação dos capacitores. Ela é analisada com auxílio da Equação \ref{vdi},
denominada VDI (voltage deviation index).
