function [s_gbest,gbest,d,media]=PSO_gbests()
% carregamento de parametros da PSO
[pso]=PSO_parametros();

% calculo do numero de mutacoes
n_mut=round(pso.p_mut*pso.n_part*pso.n_var);

% limites iniciais de velocidade p/ as particulas
S_lim(1,:)=zeros(1,pso.n_var);
S_lim(2,:)=ones(1,pso.n_var);
V_lim(1,:)=-1*ones(1,pso.n_var);
V_lim(2,:)=ones(1,pso.n_var);

for i=1:length(V_lim(1,:))
    S_k(:,i)=S_lim(2,i)-(S_lim(2,i)-S_lim(1,i)).*rand(pso.n_part,1);
    V_k(:,i)=V_lim(2,i)-(V_lim(2,i)-V_lim(1,i)).*rand(pso.n_part,1);
end

F=Funcao_Objetivo(S_k);
...
for j=1:pso.iter_max
    ...
    for i=1:n_mut
        ...
        var_sorteada=round((pso.n_var-1)*rand)+1;
