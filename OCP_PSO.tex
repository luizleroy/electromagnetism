...
Com auxílio de um método de cálculo de fluxo de carga, e a otimização por enxames de partículas, é possível construir o procedimento de alocação de dispositivos capacitivos na linha de distribuição. Os passos abaixo serão utilizados para gerar a condição inicial do algoritmo.
...
\begin{itemize}
			...
			\item Calcular fluxo de potência, tensão nas barras e as perdas elétricas para todas as partículas da geração inicial, utilizadas na construção de $\textbf{f}(x)$;
...
Durante o processo evolutivo da otimização, para um ganho de diversidade, foi elaborado um método de mutação das partículas. A uma taxa constante de probabilidade de 1\%, cada partícula pode sofrer uma alteração aleatória em seus valores. Isto ocorre para evitar que a resposta atinja, prematuramente, à um ótimo local, o que prejudicaria o resultado final. Este resultado será composto de dois importantes critérios.
...
\end{equation}
Já a variação da tensão ao longo do circuito durante a alocação dos capacitores, pode ser mensurada com auxílio da \Eq \ref{eq_vdi}, denominada VDI (\textit{Voltage Deviation Index)}, similar ao cálculo de \citeasnoun{abul2013optimal}. A estabilidade no nível de tensão, via otimização por PSO, foi explorada em \citeasnoun{kim2011optimal} e \citeasnoun{hernandez2006optimal}.
...
\begin{equation} \label{eq_multi}
  f(\textbf{x}) = w_v*\frac{VDI}{VDI_{max}} + w_p*\frac{perdas}{perdas_{max}}
\end{equation}